/* SimpleRTL8126Setup.cpp -- RTL8126 data structure initialzation methods.
*
...
bool SimpleRTL8126::initEventSources(IOService *provider)
{
    IOReturn intrResult;
    int msiIndex = -1;
    int intrIndex = 0;
    ...
    if (msiIndex != -1) {
        ...
        interruptSource = IOInterruptEventSource::interruptEventSource(this, OSMemberFunctionCast(IOInterruptEventSource::Action, this, &SimpleRTL8126::interruptHandler), provider, msiIndex);
    }
    if (!interruptSource) {
    ...
    timerSource = IOTimerEventSource::timerEventSource(this, OSMemberFunctionCast(IOTimerEventSource::Action, this, &SimpleRTL8126::timerActionRTL8126));
...
bool SimpleRTL8126::setupRxResources()
{
...
void SimpleRTL8126::refillSpareBuffers()
{
    mbuf_t m;
...
IOReturn SimpleRTL8126::refillAction(OSObject *owner, void *arg1, void *arg2, void *arg3, void *arg4)
{
...
bool SimpleRTL8126::setupTxResources()
{
    IODMACommand::Segment64 seg;
    UInt64 offset = 0;
    UInt32 numSegs = 1;
...
bool SimpleRTL8126::setupStatResources()
{
    IODMACommand::Segment64 seg;
    UInt64 offset = 0;
    UInt32 numSegs = 1;
...
void SimpleRTL8126::freeRxResources()
{
    UInt32 i;
...
void SimpleRTL8126::freeTxResources()
{
    if (txBufDesc) {
        txBufDesc->complete();
        txBufDesc->release();
...
void SimpleRTL8126::freeStatResources()
{
    if (statBufDesc) {
        statBufDesc->complete();
        statBufDesc->release();
...
void SimpleRTL8126::clearRxTxRings()
{
    mbuf_t m;
