...
const struct RTLChipInfo rtl_chip_info[] = {
        _R("RTL8126A",
        CFG_METHOD_1,
        Rx_Fetch_Number_8 | RxCfg_pause_slot_en | EnableInnerVlan | EnableOuterVlan | (RX_DMA_BURST << RxCfgDMAShift),
        0xff7e5880,
        Jumbo_Frame_9k),

        _R("RTL8126A",
        CFG_METHOD_2,
        Rx_Fetch_Number_8 | Rx_Close_Multiple | RxCfg_pause_slot_en | EnableInnerVlan | EnableOuterVlan | (RX_DMA_BURST << RxCfgDMAShift),
        0xff7e5880,
        Jumbo_Frame_9k),
...
void rtl8126_mdio_real_write_phy_ocp(struct rtl8126_private *tp,
                u16 PageNum,
                u32 RegAddr,
                u32 value)
{
...
void mdio_real_write(struct rtl8126_private *tp,
                            u16 RegAddr,
                            u16 value)
{
        if (RegAddr == 0x1F) {
...
rtl8126_enable_hw_linkchg_interrupt(struct rtl8126_private *tp)
{
        ...
        case 5:
                RTL_W32(tp, IMR_V2_SET_REG_8126, ISRIMR_V5_LINKCHG);
                break;
        case 4:
                RTL_W32(tp, IMR_V2_SET_REG_8126, ISRIMR_V4_LINKCHG);
                break;
        case 2:
        case 3:
...
rtl8126_disable_hw_interrupt(struct rtl8126_private *tp)
{
        if (tp->HwCurrIsrVer > 1) {
                ...
                if (tp->HwCurrIsrVer > 3)
                        RTL_W32(tp, IMR_V4_L2_CLEAR_REG_8126, 0xFFFFFFFF);
        } else {
...
rtl8126_irq_mask_and_ack(struct rtl8126_private *tp)
{
        ...
        if (tp->HwCurrIsrVer > 1) {
                ...
                if (tp->HwCurrIsrVer > 3)
                        RTL_W32(tp, ISR_V4_L2_8126, 0xFFFFFFFF);
        } else {
#ifdef ENABLE_DASH_SUPPORT
                if (tp->DASH) {
...
rtl8126_enable_aspm_clkreq_lock(struct rtl8126_private *tp, bool enable)
{
        ...
        case CFG_METHOD_3:
                if (enable) {
                        RTL_W8(tp, INT_CFG0_8126, RTL_R8(tp, INT_CFG0_8126) | BIT_3);
                        RTL_W8(tp, Config5, RTL_R8(tp, Config5) | BIT_0);
                } else {
                        RTL_W8(tp, INT_CFG0_8126, RTL_R8(tp, INT_CFG0_8126) & ~BIT_3);
                        RTL_W8(tp, Config5, RTL_R8(tp, Config5) & ~BIT_0);
...
rtl8126_set_eee_lpi_timer(struct rtl8126_private *tp)
{
        ...
        RTL_W16(tp, EEE_TXIDLE_TIMER_8126, dev_lpi_timer);
}
...
static const u16 phy_mcu_ram_code_8126a_1[] = {
...
static const u16 phy_mcu_ram_code_8126a_2[] = {
...
static const u16 phy_mcu_ram_code_8126a_3[] = {
...
rtl8126_real_set_phy_mcu_8126a_1(struct net_device *dev)
{
        rtl8126_set_phy_mcu_ram_code(dev,
                                     phy_mcu_ram_code_8126a_1,
                                     ARRAY_SIZE(phy_mcu_ram_code_8126a_1));
}
...
{
        rtl8126_set_phy_mcu_ram_code(dev,
                                     phy_mcu_ram_code_8126a_2,
                                     ARRAY_SIZE(phy_mcu_ram_code_8126a_2));
}

static void
rtl8126_real_set_phy_mcu_8126a_3(struct net_device *dev)
{
        rtl8126_set_phy_mcu_ram_code(dev,
                                     phy_mcu_ram_code_8126a_3,
                                     ARRAY_SIZE(phy_mcu_ram_code_8126a_3));
}
...
rtl8126_set_phy_mcu_8126a_1(struct net_device *dev)
{
        ...
        rtl8126_real_set_phy_mcu_8126a_3(dev);
...
rtl8126_set_hw_phy_before_init_phy_mcu(struct net_device *dev)
{
    ...
    switch (tp->mcfg) {
        case CFG_METHOD_1:
            IOLog("set_hw_phy_before_init_phy_mcu");
            break;
    }
}
...
rtl8126_setup_mqs_reg(struct rtl8126_private *tp)
{
        ...
        case 5:
                ...
                break;
        case 6:
                hw_clo_ptr0_reg = HW_CLO_PTR0_8126BP;
                sw_tail_ptr0_reg = SW_TAIL_PTR0_8126BP;
                reg_len = 8;
                break;
        default:
                hw_clo_ptr0_reg = HW_CLO_PTR0_8126;
...
static int rtl8126_try_msi(struct rtl8126_private *tp)
{
        ...
        hw_supp_irq_nvecs = R8126_MAX_MSIX_VEC_8126B;
